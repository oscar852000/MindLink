...
│   ├── prompts.py         # ⭐ 提示词统一管理（唯一真理源）
│   ├── routes/            # 路由模块
│   │   ├── mind.py        # Mind CRUD
│   │   ├── feed.py        # 投喂、输出、时间轴、叙事
│   │   ├── chat.py        # AI 对话
│   │   └── admin.py       # 提示词管理 API
│   └── services/          # 业务逻辑层
│       ├── ai_service.py  # AI 调用（整理、输出、叙事）
...
> ⚠️ **桌面端和移动端代码完全独立**，改一边不影响另一边。
> 
> 这样设计的原因：两端 UI 交互差异大，独立维护更清晰，后人更容易接手。
...
所有提示词集中在 `api/prompts.py`，这是**唯一真理源**：

```
┌─────────────────────────────────────────────────────────────┐
│                    prompts.py (唯一真理源)                   │
│   PROMPT_REGISTRY: 10个提示词的元数据 + 默认内容              │
│   get_prompt(key): 获取提示词（数据库 > 默认值）              │
└─────────────────────────────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        ↓                   ↓                   ↓
┌───────────────┐   ┌───────────────┐   ┌───────────────┐
│  ai_service   │   │ chat_service  │   │   admin.py    │
│  (导入使用)    │   │  (导入使用)    │   │  (导入使用)    │
└───────────────┘   └───────────────┘   └───────────────┘
```
...
|---------|-----------|-----------|
| 投喂 | `cleaner` | 投喂 |
| 叙事 | `narrative_with_meta` | 叙事 |
| 输出 | `expresser` | 输出 |
| 对话 | `chat_base` | 对话 |
| 对话风格 | `chat_style_*` | 对话风格-xxx |

**获取优先级**：数据库自定义 > prompts.py 默认值
...
| 禁止行为 | 原因 |
|----------|------|
| 在 ai_service.py 定义提示词常量 | 破坏单一真理源 |
| 在 chat_service.py 定义 DEFAULT_PROMPTS | 造成多处定义 |
| 硬编码提示词字符串 | 无法通过后台管理 |
| 绕过 get_prompt() 直接读取 | 丢失数据库优先级逻辑 |
...
| `generate_narrative_with_meta()` | 生成叙事视图 | `narrative_with_meta` |
